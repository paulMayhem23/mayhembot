...
//		usersOnDecks = data.room.metadata.djs;
		...
		if(cmbot.settings.bannedSongs[artist.toLowerCase()] != undefined) {
			if(cmbot.settings.bannedSongs[artist.toLowerCase()].indexOf(song.metadata.song.toLowerCase()) != -1) {
				cmbot.bot.speak(song.djname + ", " + song.metadata.song + " by " + artist + " is banned! Skip that shite or get kicked in 15 seconds");
			...
			if(artist !== false && track !== false) {
				...
				if(options.cmbot.settings.bannedSongs[artist].indexOf(track) > -1)
					...
					if(options.arg == "") {
						...
						options.cmbot.bot.speak(song.djname + ", " + song.metadata.song + " by " + artist + " is banned! Skip that shite or you will be booted in 15 seconds");
...
//					log("setting count: " + arg);
	...
	'about': {
		command: function(options) {
			options.cmbot.speakOrPM("@MayhemBOT " + options.cmbot.VERSION + " hacked by Paulmayhem. https://github.com/paulMayhem23/mayhembot . Based heavily on https://github.com/atomjack/cmbot", options.pm, options.userid);
		},
		modonly: false,
		pmonly: false,
		help: 'About me.'
...
//	},
	...
	'party': {
		command: function(options) {
			options.cmbot.bot.vote('up');
			options.cmbot.bot.speak("PAAAARRRRTEYYY!!! woohoo!");
		},
		modonly: false,
		pmonly: false,
		hide: false,
		help: 'I will awesome the song currently playing!'
	},
	'deccount': {
		command: function(options) {
		...
		command: function(options) {
			...
			options.cmbot.yoinkTrack(function(result){
				...
				if(result.success) {
					...
					text = "yo, I already got this wan!!";
				else
					text = result.error;
				if(options.pm)
					options.cmbot.bot.pm(text, options.userid);
