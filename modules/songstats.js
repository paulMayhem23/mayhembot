...
var customEvents = [{
	...
	event: function(cmbot, data) {
		...
		if(cmbot.settings.songstats) {
			...
			cmbot.bot.speak("Stats for " + song.metadata.song + " by " + song.metadata.artist + ": " + data.room.metadata.upvotes + " :arrow_up:, " + data.room.metadata.downvotes + " :arrow_down:, " + cmbot.session.snags + " :heart:.");
		}
		cmbot.session.snags = 0;
	}
},
